...
export default function Header() {
    ...
    return (
        <>
            {/* Sentinel div just below the top of the page */}
            <div ref={ref} className="absolute top-0 h-1 w-full" />

            <header
                className={clsx(
                    "sticky top-0 w-full z-10 h-14 flex md:grid md:grid-cols-[32px_1fr_32px] lg:grid-cols-[150px_1fr_150px] md:gap-6 lg:gap-12 items-center justify-between px-4 md:px-6 transition-colors duration-300",
                    inView
                        ? "bg-transparent"
                        : "bg-background/90 backdrop-blur-lg border-b h-[57px]"
                )}
            >
                <Link href={"/"}>
